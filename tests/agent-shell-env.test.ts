...
describe('an agent\'s shell', () => {
  test('never sees Spaces\' database, encryption key or provider keys; ordinary variables pass', async () => {
    ...
    try {
      ...
      const setup = await bash.execute('t2', { command: 'echo "compose=$COMPOSE_PROJECT_NAME bun=$BUN_INSTALL_CACHE_DIR"' })
      const text = setup.content.map((c) => c.text ?? '').join('')
      expect(text).toContain(`compose=${path.basename(cwd).toLowerCase()}`)
      expect(text).toMatch(/bun=\S*\/cache\/bun/)
      session.dispose()
    } finally {
