...
The deprecated `files:read` umbrella scope is also being replaced with Figma's granular read-only scopes — `file_content:read` (file/node contents), `library_content:read` (published components & styles), and `current_user:read` (the `/v1/me` identity check) — matching exactly the three Figma endpoints the read-only tools call and requesting the least privilege Figma still supports.
...
- **FR-002**: The system MUST request the granular read scopes the read-only design tools require — `file_content:read` (file/node contents), `library_content:read` (published styles & components), and `current_user:read` (identity verification) — instead of the deprecated `files:read` umbrella scope.
...
- **FR-005**: The administrator-facing guidance for the Figma OAuth app MUST list exactly the granular scopes the system requests (`file_content:read`, `library_content:read`, `current_user:read`), so administrators enable only real, matching scopes.
