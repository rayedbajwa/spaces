...
import { syncProjectIntentsQuietly } from './lib/intent-store'
  ...
  try {
    engine = new PipelineEngine(run.templateJson, options, {
      ...
      onStageHandoff: async (h) => {
        // The stage's documents are the intent's record: sync them into the
        // database. Awaited, so syncs follow stage boundaries in order and an
        // earlier, slower one cannot overwrite a later stage's documents.
